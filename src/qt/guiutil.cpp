...
{
    // Convert memetic:// to memetic:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("memetic://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "memetic:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Memetic.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Memetic.lnk
...
{
    return GetAutostartDir() / "memetic.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a memetic.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the memetic app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add memetic app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("memetic-Qt") + " " + tr("version") + " " +
