    ...
    filter(0)
{
    ...
    if(fLiteMode){
        ...
        if(fMasterNode){
            ...
            if(!fEnableDarksend){
                ui->toggleDarksend->setText(tr("Start Smugsend Mixing"));
            } else {
                ui->toggleDarksend->setText(tr("Stop Smugsend Mixing"));
            }
...
void OverviewPage::darkSendStatus()
{
    ...
    QString s = tr("Last Smugsend message:\n") + strStatus;

    if(s != ui->darksendStatus->text())
        LogPrintf("Last Smugsend message: %s\n", strStatus.toStdString());
...
void OverviewPage::darksendReset(){
    ...
    QMessageBox::warning(this, tr("Smugsend"),
        tr("Smugsend was successfully reset."),
...
void OverviewPage::toggleDarksend(){
    ...
    if(hasMixed.isEmpty()){
        QMessageBox::information(this, tr("Smugsend"),
                tr("If you don't want to see internal Smugsend fees/transactions select \"Received By\" as Type on the \"Transactions\" tab."),
                QMessageBox::Ok, QMessageBox::Ok);
    ...
    if(!fEnableDarksend){
        ...
        if(balance < minAmount){
            ...
            QMessageBox::warning(this, tr("Smugsend"),
                tr("Smugsend requires at least %1 to use.").arg(strMinAmount),
                QMessageBox::Ok, QMessageBox::Ok);
            return;
        }
        ...
        if (walletModel->getEncryptionStatus() == WalletModel::Locked)
        {
            ...
            if(!ctx.isValid())
            {
                ...
                QMessageBox::warning(this, tr("Smugsend"),
                    tr("Wallet is locked and user declined to unlock. Disabling Smugsend."),
                    QMessageBox::Ok, QMessageBox::Ok);
                if (fDebug) LogPrintf("Wallet is locked and user declined to unlock. Disabling Smugsend.\n");
                return;
            }
        }
