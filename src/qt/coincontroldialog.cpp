        ...
        if (item->checkState(COLUMN_CHECKBOX) == Qt::Unchecked)
            ...
            if(coinControl->useDarkSend && rounds < nDarksendRounds) {
                QMessageBox::warning(this, windowTitle(),
                    tr("Non-anonymized input selected. <b>Smugsend will be disabled.</b><br><br>If you still want to use Smugsend, please deselect all non-nonymized inputs first and then check Smugsend checkbox again."),
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    l4->setText(::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
